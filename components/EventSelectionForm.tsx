  ...
  return (
    <form action={action} className="mt-6">
      ...
      <p className="mt-2 text-sm font-medium text-stone-500">
        Pick Water Quality, Ecology, Entomology, Anatomy & Physiology, or Crime
        Busters to start practicing. Other events are coming later.
      </p>
        ...
        {events.map((event) => {
          ...
          if (locked) {
            return (
              <div
                key={event.id}
                className="rounded-3xl border border-dashed border-stone-300 bg-stone-50 px-4 py-4 text-left"
              >
                <span className="flex items-start gap-3">
                  <span
                    className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${accent.wrap} ${accent.icon}`}
                  >
